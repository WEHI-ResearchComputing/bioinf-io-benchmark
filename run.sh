...
# Align
bwa mem -M -t `nproc` $REFERENCE ${WORK_DIR}/output-trimmed_1P.fastq.gz ${WORK_DIR}/output-trimmed_2P.fastq.gz > ${WORK_DIR}/aln.sam

# Sort
samtools sort -@ `nproc` ${WORK_DIR}/aln.sam > ${WORK_DIR}/aln.bam
